...
analisisJSON macro buffer
    ...
    CICLO:
        ...
        cmp dh, 60                 ; Se verifica si el caracter en el registro dh es igual a las comillas dobles '<'
    ...
    IDS:
        inc si                      ; Se incremente el contador guardado en el registro SI 
        mov dh, buffer[si]          ; Se obtiene el caracter del buffer en la posicion actual de registro si y se guarda en el registro dh
        cmp dh, 62                 ; Se verifica si el caracter en el registro dh es igual a las comillas dobles '"' >
        je VERIFICARID              ; Si son comillas dobles se salta a la etiqueta para guardar IDs

        cmp dh, 86                 ; Se verifica si el caracter en el registro dh es igual al signo numeral 'V'
        je BUSCARNUM                ; Si son comillas dobles se salta a la etiqueta para reconocer numeros
        cmp dh, 118                 ; Se verifica si el caracter en el registro dh es igual al signo numeral 'v'
        je BUSCARNUM 
    ...
    BUSCARNUM:
        ESPNUMC:    
            inc si    ;"#": <valor> -40 </valor>,
            mov dh, buffer[si]
            cmp dh, 62 ; COMILLAS DOBLES >
            je ESPNUMC
    ...
    CICLONUM:
        ...
        xor si, si
        mov si, cx
        mov auxiliar[si], dh ;//guarda el caracter (num)
        inc cx
        POP si
        jmp CICLONUM
    NEGARNUMERO:
